...
void setup()
{
  ...
  pinMode(L3_Count_TRIG, OUTPUT);
  pinMode(L3_Count_ECHO, INPUT);
  pinMode(L3_Speed_TRIG, OUTPUT);
  pinMode(L3_Speed_ECHO, INPUT);
  pinMode(L4_Count_TRIG, OUTPUT);
  pinMode(L4_Count_ECHO, INPUT);
  pinMode(L4_Speed_TRIG, OUTPUT);
  pinMode(L4_Speed_ECHO, INPUT);
...
void loop()
{
  ...
  if (d3 < 20.0 && !L3_detected)
  {
    count_L3++;
    L3_detected = true;
    Serial.print("COUNT:L3:");
    Serial.println(count_L3);
  }
  if (d3 >= 20.0)
    L3_detected = false;
  ...
  if (d4 < 20.0 && !L4_detected)
  {
    count_L4++;
    L4_detected = true;
    Serial.print("COUNT:L4:");
    Serial.println(count_L4);
  }
  if (d4 >= 20.0)
    L4_detected = false;
